...
func (r *repository) GetByID(ctx context.Context, walletID uuid.UUID) (*Wallet, error) {
	var wallet Wallet
	err := r.db.WithContext(ctx).Model(&Wallet{}).Where("id = ? AND deleted_at IS NULL", walletID).First(&wallet).Error
	if err != nil {
		return nil, err
	}
...
func (r *repository) GetByNameAndUserID(ctx context.Context, name string, userID uuid.UUID) (*Wallet, error) {
	var wallet Wallet
...
func (r *repository) Delete(ctx context.Context, walletID uuid.UUID) error {
	result := r.db.WithContext(ctx).Delete(&Wallet{}, walletID)
	if result.Error != nil {
		return result.Error
	}
